...
def send_message(server_address, message: Dict[str, str]) -> Dict[str, str]:
    ...
    if len(completed.stderr) != 0:
        print(completed.stderr)  # print to ease debugging
...
def get_messages(server_address, waiter: Waiter) -> List[Dict]:
    ...
    lines = []
    while completed.stdout.peek():
        line = completed.stdout.readline().decode('ascii')
        lines.append(line)
...
def grpcurl_output_str_to_dict_list(output_str: str) -> List[Dict]:
    output_str_list = output_str.strip().split('{\n')[1:]
    output_dict_list = [ast.literal_eval('{' + output) for output in output_str_list]
