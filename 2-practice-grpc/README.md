...
Названия сервиса, методов сервиса и полей сообщений в прото файле должны называться и иметь такие же типы как в примерах, иначе тесты не пройдут. Названия сообщений в прото файле должны быть: `ClientMessage`, `SendMessageResponse`, `IdentificationMessage`, `ServerMessage`. Поле `client_id` должно иметь тип `uint32`, поле `sendTime` должно иметь тип `google.protobuf.Timestamp`. Для использования этого типа необходимо прописать импорт в прото файле: `import "google/protobuf/timestamp.proto";`.
2. Скомпилируйте прото файл с помощью `protoc` в нужный для вас язык. Для Python, из папки `2-practice-grpc`:
