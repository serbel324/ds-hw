...
**Как измеряется потребление памяти в тестах на overhead?**

См. [здесь](https://github.com/osukhoroslov/dslib/blob/2e603300324f99402b32824e86f9ada0a1f41696/src/pynode.rs#L222). Данной функции передается объект Sender или Receiver. Функция вызывается периодически по ходу выполнения теста, и запоминается максимальное полученное значение, которое выводится в конце теста.

**Можно ли хранить данные в глобальных переменных?**

В тестах на overhead измеряется потребление памяти только структурами данных (атрибутами) внутри объектов Sender и Receiver. Не используйте глобальные переменные для хранения мутабельных состояний узлов. Это будет рассматриваться как чит, и пункт с оптимизацией потребляемых ресурсов засчитан не будет.

**Достаточно ли для прохождения тестов на overhead стандартных библиотек?**

Да. Для прохождения тестов на overhead не требуются сторонние библиотеки типа numpy, сжатие и распаковка данных, специальные бинарные форматы и т.п.
