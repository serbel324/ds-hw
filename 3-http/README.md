...
При проверке будет передаваться заголовок `Accept-Encoding: gzip`, в ответе ожидается `Accept-Encoding: gzip`, в теле ответа должны передаваться данные, сжатые алгоритмом gzip.
...
Вам нужно реализовать HTTP сервер без использования готовых библиотек доступных в интернете или в стандартной библиотеке вашего языка.

Код заготовки на питоне приведен в файле `server/server.py`. Вы можете написать решение на основе этого кода, либо использовать любой другой язык.

Тесты написаны на Go, для запуска скачайте и установите последнюю версию с сайта https://go.dev/dl/.
Выполнение тестов начинается с функции `TestHW`, которая находится в файле `hw_test.go`.
...
Для запуска решения при локальном тестировании используется файл `./launch.sh`. Если вы хотите использовать другую версию Python или по другому запускать решение, можно отредактировать этот файл.
