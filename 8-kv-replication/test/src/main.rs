...
fn send_put(sys: &mut System<JsonMessage>, node: &str, key: &str, value: &str, quorum: u8) {
    sys.send_local(
        JsonMessage::from("PUT", &PutMessage { key, value, quorum }),
        node,
    );
...
fn check_delete(
    ...
    sys.send_local(
        JsonMessage::from("DELETE", &DeleteMessage { key, quorum }),
        node,
    );
    let res = sys.step_until_local_message_max_steps(node, max_steps);
    assume!(
...
fn test_basic(config: &TestConfig) -> TestResult {
    ...
    // get key from the first node
    check_get(&mut sys, &nodes[0], &key, 2, Some(&value2), 100)?;
...
fn test_sloppy_quorum_read(config: &TestConfig) -> TestResult {
    ...
    sys.disconnect_node(&replicas[1]);
...
fn test_sloppy_quorum_tricky(config: &TestConfig) -> TestResult {
    ...
    sys.disconnect_node(&replicas[1]);
    sys.disconnect_node(&replicas[2]);
    sys.disconnect_node(&non_replicas[0]);
