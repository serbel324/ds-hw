...
fn add_node(node_id: &str, sys: &mut System<JsonMessage>, config: &TestConfig) {
    ...
    let node = config.node_factory.build(node_id, (node_id, node_ids), config.seed);
    sys.add_node(rc!(refcell!(node)));
}

fn check_get(sys: &mut System<JsonMessage>, node: &str, key: &str,
...
fn check_delete(sys: &mut System<JsonMessage>, node: &str, key: &str,
             expected: Option<&str>, max_steps: u32) -> TestResult {
...
fn dump_keys(sys: &mut System<JsonMessage>, node: &str) -> Result<HashSet<String>, String> {
    ...
    assume!(res.is_ok(), format!("DUMP_KEYS_RESP is not returned by {}", node))?;
    let msgs = res.unwrap();
    let msg = msgs.first().unwrap();
...
fn key_distribution(sys: &mut System<JsonMessage>) -> Result<HashMap<String, HashSet<String>>, String> {
...
fn count_records(sys: &mut System<JsonMessage>, node: &str) -> Result<u64, String> {
    sys.send_local(JsonMessage::from("COUNT_RECORDS", &CountRecordsMessage {}), node);
    let res = sys.step_until_local_message_max_steps(node, 100);
    assume!(res.is_ok(), format!("COUNT_RECORDS_RESP is not returned by {}", node))?;
    let msgs = res.unwrap();
    let msg = msgs.first().unwrap();
...
fn send_node_added(sys: &mut System<JsonMessage>, added: &str) {
    for node in sys.get_node_ids() {
        sys.send_local(JsonMessage::from("NODE_ADDED", &NodeAddedMessage {id: added}), &node);
    }
}

fn send_node_removed(sys: &mut System<JsonMessage>, removed: &str) {
    for node in sys.get_node_ids() {
        sys.send_local(JsonMessage::from("NODE_REMOVED", &NodeRemovedMessage {id: removed}), &node);
    }
}

fn step_until_stabilized(sys: &mut System<JsonMessage>, nodes: &Vec<String>, expected_keys: u64,
    ...
    assume!(stabilized,
        format!("Keys distribution is not stabilized (keys observed = {}, expected = {})",
            total_count, expected_keys))
}

fn check(sys: &mut System<JsonMessage>, nodes: &Vec<String>, expected: &HashMap<String, String>,
         check_values: bool, check_distribution: bool) -> TestResult {
    ...
    if check_distribution {
        ...
        let deviations: Vec<f64> = node_key_counts.iter()
            .map(|x| (target_count - *x as f64).abs() / target_count).collect();
...
fn check_moved_keys(sys: &mut System<JsonMessage>, before: &HashMap<String, HashSet<String>>,
                    after: &HashMap<String, HashSet<String>>, target: u64) -> TestResult {
...
fn test_node_removed(config: &TestConfig) -> TestResult {
    ...
    // run the system until key the distribution is stabilized
    let nodes: Vec<String> = sys.get_node_ids().into_iter().filter(|x| *x != removed).collect();
    step_until_stabilized(&mut sys, &nodes, kv.len() as u64, 100, 1000)?;
...
fn test_node_removed_after_crash(config: &TestConfig) -> TestResult {
    ...
    assume!(crashed_keys.len() > 0, "Node stores no records, bad distribution")?;
    ...
    // run the system until key the distribution is stabilized
    let nodes: Vec<String> = sys.get_node_ids().into_iter().filter(|x| *x != crashed).collect();
    step_until_stabilized(&mut sys, &nodes, kv.len() as u64, 100, 1000)?;
...
fn main() {
    let matches = App::new("Sharded KV Store Homework Tests")
    ...
    config.node_count = 1;
    tests.add("SINGLE NODE", test_single_node, config);
